...
class BaseAdminObject(object):
    ...
    def get_view(self, view_class, option_class=None, *args, **kwargs):
        #print('get_view ==== base')        
        """
        注意：在此之前 ~xadmin.sites.AdminSite.get_view_class` 方法  已经运行
...
class CommAdminView(BaseAdminView):
    ...
    **View属性**:
    ...
    site_title = getattr(settings, "XADMIN_TITLE", _("Xadmin"))  #: 网站的标题
    site_footer = getattr(settings, "XADMIN_FOOTER_TITLE", _("my-company.inc"))  #: 网站的下角标文字
    #: 全局的 Model 图标::
    #:
